jQuery(function(b){
				...
				b.ajax({
					...
					success:function(g){
						...
						if(b("#search").val()!=""){
							...
							b("#search_autocomplete").css({display:"block"}).fadeIn("fast");
	...
	b("#search").click(function(){
		if(b("#search").val()!=""&&b("#search").val()!="Search entire store here..."){
			c=true;b("#search_autocomplete").css({display:"block"}).fadeIn("fast")
			}else{
				b("#search").val("")}});
	b("#search_autocomplete").mouseout(function(){
		c=false;a=setTimeout(function(){
			if(c!=true){
				b("#search_autocomplete").fadeOut("slow")
				}else{
					clearTimeout(a);
					c=true}},1500)}).mouseover(function(){
						clearTimeout(a);c=true});
